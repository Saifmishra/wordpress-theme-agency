	...
	public function ParseRIFF($startoffset, $maxoffset) {
		...
		try {
			...
			while ($this->ftell() < $maxoffset) {
				...
				if (($chunksize % 2) != 0) {
					// all structures are packed on work boundaries
					$chunksize++;
				}
							...
							case 'data':
								...
								if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', substr($testData, 0, 4))) {
									...
									if (empty($getid3_temp->info['error'])) {
										...
										$info['playtime_seconds'] = $getid3_temp->info['playtime_seconds']; // may not match RIFF calculations since DTS-WAV often used 14/16 bit-work packing
										if (!empty($getid3_temp->info['warning'])) {
											foreach ($getid3_temp->info['warning'] as $newerror) {
