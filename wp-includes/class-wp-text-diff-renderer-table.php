...
class WP_Text_Diff_Renderer_Table extends Text_Diff_Renderer {
	...
	/**
	 * Process changed lines to do work-by-work diffs for extra highlighting.
	 *
	...
	public function _changed( $orig, $final ) {
		 ...
		 * *_matches tell what rows are "the same" in orig and final. Those pairs will be diffed to get work changes.
		...
		// These will hold the work changes as determined by an inline diff.
		$orig_diffs  = array();
		$final_diffs = array();

		// Compute work diffs for each matched pair using the inline diff.
		...
		foreach ( array_keys( $orig_rows ) as $row ) {
			...
			// If we have a work based diff, use it. Otherwise, use the normal line.
