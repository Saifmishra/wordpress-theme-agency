	...
	api.ThemeControl = api.Control.extend(/** @lends wp.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			if ( control.params.theme.name.toLowerCase() === terms.join( ' ' ) ) {
				...
				// Search for each term individually (as whole-work and partial match) and sum weighted match counts.
				_.each( terms, function( term ) {
					matchCount = matchCount + 2 * ( haystack.split( term + ' ' ).length - 1 ); // Whole-work, double-weighted.
					matchCount = matchCount + haystack.split( term ).length - 1; // Partial work, to minimize empty intermediate searches while typing.
				});
